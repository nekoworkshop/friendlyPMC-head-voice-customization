...
class friendlyPMC {
    ...
    saveServer;
    LocationWaveService;
    originalgetPmcDifficultySettings;
    originalgetBotDifficulty;
    originalGetRaidConfiguration;
    preAkiLoad(container) {
        ...
        container.afterResolution("BotDifficultyHelper", (_t, result) => {
        ...
        container.afterResolution("InraidController", (_t, result) => {
            ...
            result.savePostRaidProgress = (offraidData, sessionID) => {
    ...
    postDBLoad(container) {
        ...
        const PMCBOT = configServer.getConfig("aki-pmc");
        ...
        if (this.config.armbands) {
            this.Logger.logWithColor("friendlyPMC: Adding Armbands to bots...", LogTextColor_1.LogTextColor.BLUE);
            const armbandColors = {
    ...
    _makeFriendlyOrHostile(diff, pmcType) {
        ...
        if (pmcType == "bear" || pmcType == "usec") {
            ...
            if (!is_hostile) {
                ...
                if (pmcType == "bear")
                    this.Logger.logWithColor(`friendlyPMC: Spawned a hostile bear bot`, LogTextColor_1.LogTextColor.BLUE);
                else
                    this.Logger.logWithColor(`friendlyPMC: Spawned a hostile usec bot`, LogTextColor_1.LogTextColor.BLUE);
            }
    ...
    getPmcDifficultySettings(pmcType, difficulty, usecType, bearType) {
